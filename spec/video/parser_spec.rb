...
RSpec.describe Parser do
  context(:example_dataset) do
    ...
    it 'parses the first endpoint caches' do
      ...
      expect(parser.endpoints[0].cache_connections[0].latency_delta).to eq(900)
      ...
      expect(parser.endpoints[0].cache_connections[1].latency_delta).to eq(800)
      ...
      expect(parser.endpoints[0].cache_connections[2].latency_delta).to eq(700)
  ...
  context(:large_datasets) do
    def also_works_with(file_name)
      Parser.new("video/datasets/#{file_name}.in").parse
    end

    it { also_works_with('kittens') }
    it { also_works_with('me_at_the_zoo') }
    it { also_works_with('trending_today') }
    it { also_works_with('videos_worth_spreading') }
  end
end
